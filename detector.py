...
try:
    ...
    imlist = []
    imlist.append(osp.join(osp.relpath('.'), images))
except FileNotFoundError:
    print("No file or directory with the name {}".format(images))
    exit()

if not os.path.exists(args.det):  # if detection directory doesn't exits, create one
    os.makedirs(args.det)

load_batch = time.time()

# creates a list of images as np arrays
loaded_ims = [cv2.imread(x) for x in imlist]


# apply the prep_image function to all loaded_ims
im_batches = list(
    map(
        prep_image,
        loaded_ims,
...
if batch_size != 1:
    numbatches = len(imlist) // batch_size + leftover
    im_batches = [
        torch.cat(
...
for i, batch in enumerate(im_batches):
    ...
    # transform attribute from index in batch to index in imlist
...
# resize detections to original image dimensions
...
# undo letterbox rescaling / padding on bounding box dimensions
output[:, 1:5] /= scaling_factor

# clip bboxes with edges outside the resized image
...
# apply the write function over all outputs
list(map(lambda x: write(x, loaded_ims), output))

# save all images with prefix "det_" and write them to det directory
